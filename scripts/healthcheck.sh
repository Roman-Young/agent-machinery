...
echo "═══ Kairo healthcheck — $(date '+%F %H:%M %Z') ═══"
...
if [[ $FAIL -eq 0 && $WARN -eq 0 ]]; then echo "  ✅ Kairo is healthy on all five properties."
...
[[ $FAIL -gt 0 ]] && "$SCRIPT_DIR/notify.sh" alert "❌ Kairo healthcheck: $FAIL failure(s)" \
