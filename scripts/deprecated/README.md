...
It **collided** with `cairn-on-mac-install.sh`: both wrote `~/.cairn/sync.sh` and both
...
Everything it did is now folded into **`cairn-on-mac-install.sh`**, which is the single Mac
