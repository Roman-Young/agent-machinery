...
Two Kairos exist — one on the server, one in the laptop's editor. They **share files**; they
do not talk.

Two Kairos, a clean division of labour. **Simplified 2026-07-15: NO code crosses between
...
```
   ...
   │    DOWN)            │              │  journal, phone     │
   └──────────┬──────────┘              └──────────┬──────────┘
              │  memory       ◀──── rsync ─────────┤  DOWN: Kairo knows you in VS Code
...
Why no code sync? Because once Kairo is IN VS Code, it has your files natively — mirroring
...
server). VS Code Kairo *requests* changes via the outbox; it never writes memory directly.
